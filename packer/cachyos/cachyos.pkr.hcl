# CachyOS (基于 Arch Linux) 定制镜像构建 - HashiCorp Packer 版本
...
locals {
  ...
  output_filename = "CachyOS-NEXT.qcow2"
}
...
build {
  ...
  provisioner "shell" {
    inline = [
      "cat <<'MIRROREOF' | sudo tee /etc/pacman.d/cachyos-mirrorlist",
      ...
      "MIRROREOF",
      "cat <<'MIRROREOF' | sudo tee /etc/pacman.d/cachyos-v4-mirrorlist",
