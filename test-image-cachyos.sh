...
# CachyOS (基于 Arch Linux) 定制镜像全面测试脚本
# 创建时间: 2025-12-13 +08:00
# 创建者: Mare Ashley Pecker (mare@sent.com)
# 用途: 验证 imagepacker-cachyos.sh 构建的镜像的所有定制修改
...
if [ ! -f /etc/os-release ]; then
    ...
    echo "⚠️  此脚本必须在 CachyOS/Arch Linux 虚拟机中运行，不能在宿主机上运行！"
    echo ""
    echo "正确的使用方法："
    echo "1. 启动 CachyOS 虚拟机"
    echo "2. 将此脚本上传到虚拟机中"
    echo "3. 在虚拟机内运行: ./test-image-cachyos.sh"
    echo ""
    echo "当前系统: $(uname -s)"
    exit 1
fi

# 检查是否是 Arch Linux 系统（CachyOS 基于 Arch）
if ! grep -qE "ID=arch|ID_LIKE=.*arch" /etc/os-release 2>/dev/null; then
    echo "⚠️  警告: 此脚本是为 CachyOS (基于 Arch Linux) 设计的"
...
# 测试 5: CachyOS 内核验证
# ============================================
start_test "CachyOS 内核验证"

log_section "5.1 当前运行内核"
KERNEL_VERSION=$(uname -r)
log_info "内核版本: ${KERNEL_VERSION}"
assert_contains "uname -r" "cachyos" "CachyOS内核正在运行"

log_section "5.2 已安装CachyOS内核包"
log_step "列出所有内核包"
pacman -Q | grep -E "^linux-cachyos" | tee -a "$TEST_LOG"
assert_contains "pacman -Q linux-cachyos" "linux-cachyos" "linux-cachyos内核包已安装"
...
if grep -q "\[cachyos\]" /etc/pacman.conf && grep -q "\[cachyos-v4\]" /etc/pacman.conf; then
    ...
    log_error "CachyOS仓库配置不完整"
    prompt_continue
fi

log_section "5.5 CachyOS设置包"
if pacman -Q cachyos-settings &>/dev/null; then
    log_success "cachyos-settings已安装"
    pacman -Q cachyos-settings | tee -a "$TEST_LOG"
else
    log_warning "cachyos-settings未安装"
fi

log_section "5.6 SCX调度器"
if pacman -Q scx-scheds &>/dev/null; then
    log_success "scx-scheds已安装"
    pacman -Q scx-scheds | tee -a "$TEST_LOG"
else
    log_warning "scx-scheds未安装"
fi

log_section "5.7 内核详细信息"
log_info "完整内核信息:"
cat /proc/version | tee -a "$TEST_LOG"

end_test "CachyOS 内核验证"
...
**生成者**: test-image-cachyos.sh v1.0
