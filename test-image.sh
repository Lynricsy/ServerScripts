...
# Debian 13 定制镜像全面测试脚本
# 创建时间: 2025-10-23 18:04:33 +08:00
# 创建者: Mare Ashley Pecker (mare@sent.com)
# 用途: 验证 imagepacker.sh 构建的镜像的所有定制修改
...
if [ ! -f /etc/os-release ]; then
    ...
    echo "⚠️  此脚本必须在 Debian/Linux 虚拟机中运行，不能在宿主机上运行！"
    echo ""
    echo "正确的使用方法："
    echo "1. 启动 Debian 虚拟机"
    echo "2. 将此脚本上传到虚拟机中"
    echo "3. 在虚拟机内运行: ./test-image.sh"
    echo ""
    echo "当前系统: $(uname -s)"
    exit 1
fi
...
assert_command_exists() {
    ...
    if command -v "$cmd" &> /dev/null; then
        local version=$(eval "$cmd --version 2>&1 | head -1" || echo "已安装")
...
assert_contains "cat /etc/os-release" "Debian GNU/Linux 13" "系统为 Debian 13"
assert_contains "cat /etc/os-release" "trixie" "代号为 trixie"
...
assert_contains "cat /etc/default/grub" "GRUB_DEFAULT=0" "默认启动项为0"
assert_contains "cat /etc/default/grub" "GRUB_TIMEOUT=5" "超时时间为5秒"
assert_contains "cat /etc/default/grub" "GRUB_DISABLE_SUBMENU=y" "禁用子菜单"

log_section "3.2 Serial Console服务"
...
NETWORK_TOOLS=("ping:iputils-ping" "mtr:mtr" "netstat:net-tools")
...
assert_command_exists "batcat" "batcat"
assert_command_exists "fdfind" "fd-find"
...
# 测试 5: Xanmod内核验证
# ============================================
start_test "Xanmod内核验证"

log_section "5.1 当前运行内核"
KERNEL_VERSION=$(uname -r)
log_info "内核版本: ${KERNEL_VERSION}"
assert_contains "uname -r" "xanmod" "Xanmod内核正在运行"

log_section "5.2 已安装内核包"
log_step "列出所有内核包"
dpkg -l | grep linux-image | tee -a "$TEST_LOG"
assert_contains "dpkg -l | grep linux-image" "xanmod" "Xanmod内核包已安装"

log_section "5.3 验证旧内核已删除"
log_step "检查是否残留cloud内核"
if dpkg -l | grep -q "linux-image.*cloud-amd64"; then
    log_warning "发现残留的cloud内核包"
    dpkg -l | grep "linux-image.*cloud-amd64" | tee -a "$TEST_LOG"
else
    log_success "旧的cloud内核已清理"
fi

log_section "5.4 Xanmod仓库配置"
assert_file_exists "/etc/apt/sources.list.d/xanmod-release.list" "Xanmod仓库配置"
assert_contains "cat /etc/apt/sources.list.d/xanmod-release.list" "xanmod.org" "Xanmod仓库正确配置"

log_section "5.5 内核详细信息"
log_info "完整内核信息:"
...
assert_file_exists "/etc/sysctl.d/99-network-optimization.conf" "网络优化配置文件"
log_step "显示网络优化配置"
cat /etc/sysctl.d/99-network-optimization.conf | tee -a "$TEST_LOG"
...
assert_success "systemctl is-active containerd.service" "Containerd服务运行中"
assert_success "systemctl is-enabled docker.service" "Docker服务已启用"
assert_success "systemctl is-enabled containerd.service" "Containerd服务已启用"
...
assert_contains "zsh /tmp/check_aliases.zsh" "fdfind" "find别名使用fdfind"
...
log_step "使用bat显示文件"
batcat --paging=never --style=plain /etc/hostname 2>&1 | tee -a "$TEST_LOG"
log_success "bat运行正常"

log_section "9.4 测试fd功能"
log_step "使用fdfind查找文件"
fdfind -d 1 -t f "hostname" /etc 2>&1 | head -3 | tee -a "$TEST_LOG"
log_success "fdfind运行正常"
...
log_section "11.1 APT缓存清理"
log_step "检查APT缓存大小"
APT_CACHE_SIZE=$(du -sh /var/cache/apt/ 2>/dev/null | cut -f1)
log_info "APT缓存大小: ${APT_CACHE_SIZE}"

APT_ARCHIVES_COUNT=$(ls /var/cache/apt/archives/*.deb 2>/dev/null | wc -l)
log_info "缓存包数量: ${APT_ARCHIVES_COUNT}"
if [ "$APT_ARCHIVES_COUNT" -le 5 ]; then
    log_success "APT缓存已清理"
else
    log_warning "APT缓存未完全清理（${APT_ARCHIVES_COUNT}个包）"
fi

log_section "11.2 APT列表清理"
log_step "检查APT列表大小"
APT_LISTS_SIZE=$(du -sh /var/lib/apt/lists/ 2>/dev/null | cut -f1)
log_info "APT列表大小: ${APT_LISTS_SIZE}"

log_section "11.3 日志文件清理"
...
log_section "12.2 APT源配置"
assert_file_exists "/etc/apt/sources.list.d/debian.sources" "Debian APT源"
log_step "检查源配置"
if ! grep -q "deb-src" /etc/apt/sources.list.d/debian.sources 2>/dev/null; then
    log_success "源代码仓库已禁用"
else
    log_warning "源代码仓库可能未禁用"
fi

log_section "12.3 Cloud-init配置"
if [ -f "/etc/cloud/cloud.cfg.d/01_debian_cloud.cfg" ]; then
    assert_file_exists "/etc/cloud/cloud.cfg.d/01_debian_cloud.cfg" "Cloud-init配置"
    if grep -q "generate_mirrorlists: false" /etc/cloud/cloud.cfg.d/01_debian_cloud.cfg; then
        log_success "Mirror列表生成已禁用"
    else
        log_warning "Mirror列表生成可能未禁用"
    fi
else
    log_info "Cloud-init配置文件不存在（可能是预期行为）"
fi

log_section "12.4 QEMU Guest Agent"
...
JOURNAL_FILTERED=$(echo "$JOURNAL_ALL" | grep -Ev "shpchp .*pci_hp_register failed|Slot initialization failed|snd_hda_intel .*no codecs found|agetty\[.*\]: .*failed to get terminal attributes" || true)
...
log_section "14.1 DPKG 数据库审计"
DPKG_AUDIT_OUT=$(dpkg --audit 2>&1 || true)
if [ -z "${DPKG_AUDIT_OUT}" ]; then
    log_success "dpkg --audit 未发现异常"
else
    log_error "dpkg --audit 发现问题"
    echo "${DPKG_AUDIT_OUT}" | tee -a "$TEST_LOG"
    prompt_continue
fi

log_section "14.2 APT 数据库一致性"
assert_success "apt-get -o Debug::NoLocking=1 -y check" "APT数据库一致性"

log_section "14.3 DPKG/APT 锁文件"
LOCKS=("/var/lib/dpkg/lock-frontend" "/var/lib/dpkg/lock" "/var/cache/apt/archives/lock")
for lock in "${LOCKS[@]}"; do
    if [ -e "$lock" ]; then
        HOLDERS=""
        if command -v fuser >/dev/null 2>&1; then
            HOLDERS=$(fuser -u "$lock" 2>/dev/null | tr -d ' ')
        fi
        if [ -z "$HOLDERS" ] && command -v lsof >/dev/null 2>&1; then
            HOLDERS=$(lsof -t "$lock" 2>/dev/null | tr -d ' ')
        fi
        if [ -n "$HOLDERS" ]; then
            log_error "APT/DPKG锁被占用: $lock (PIDs: $HOLDERS)"
            echo "相关进程:"
            ps -o pid,ppid,cmd -p $(echo "$HOLDERS" | tr ' ' '\n' | tr -s '\n' ' ') 2>/dev/null | tee -a "$TEST_LOG"
            prompt_continue
        else
            log_warning "发现残留锁文件但未被占用: $lock"
        fi
    fi
done
if systemctl list-units --type=service --all 2>/dev/null | grep -qE '^apt-daily(\.|@)'; then
    if systemctl is-active apt-daily.service >/dev/null 2>&1 || systemctl is-active apt-daily-upgrade.service >/dev/null 2>&1; then
        log_warning "apt-daily 正在运行，可能临时持有锁"
    fi
fi
...
assert_success "getent hosts deb.debian.org" "DNS解析 deb.debian.org"
...
if systemctl list-unit-files 2>/dev/null | grep -qE '^ssh\.service'; then
    if systemctl is-enabled ssh >/dev/null 2>&1; then
        ...
        log_warning "ssh 服务未启用"
    fi
    if systemctl is-active ssh >/dev/null 2>&1; then
        log_success "ssh 服务运行中"
    else
        log_warning "ssh 服务未运行"
    fi
else
    log_info "未安装 openssh-server（可忽略）"
fi
