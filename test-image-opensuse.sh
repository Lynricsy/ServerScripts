...
# openSUSE Tumbleweed 定制镜像全面测试脚本
# 创建时间: 2025-12-13 +08:00
# 创建者: Mare Ashley Pecker (mare@sent.com)
# 用途: 验证 imagepacker-opensuse.sh 构建的镜像的所有定制修改
...
if [ ! -f /etc/os-release ]; then
    ...
    echo "⚠️  此脚本必须在 openSUSE/Linux 虚拟机中运行，不能在宿主机上运行！"
    echo ""
    echo "正确的使用方法："
    echo "1. 启动 openSUSE 虚拟机"
    echo "2. 将此脚本上传到虚拟机中"
    echo "3. 在虚拟机内运行: ./test-image-opensuse.sh"
    echo ""
    echo "当前系统: $(uname -s)"
    exit 1
fi
...
# 检查是否是 openSUSE 系统
if ! grep -qE "ID.*=.*opensuse|ID_LIKE=.*suse" /etc/os-release 2>/dev/null; then
    echo "⚠️  警告: 此脚本是为 openSUSE Tumbleweed 设计的"
...
log_header "🎉 openSUSE Tumbleweed 定制镜像全面测试 🎉"
...
assert_contains "cat /etc/os-release" "openSUSE" "系统为 openSUSE"
assert_contains "cat /etc/os-release" "Tumbleweed" "版本为 Tumbleweed"
...
log_section "3.2 GRUB2配置生成"
assert_file_exists "/boot/grub2/grub.cfg" "GRUB2配置文件"
...
log_step "列出所有内核包"
rpm -qa | grep kernel | tee -a "$TEST_LOG"
...
log_section "11.1 Zypper缓存清理"
log_step "检查Zypper缓存大小"
ZYPP_CACHE_SIZE=$(du -sh /var/cache/zypp/ 2>/dev/null | cut -f1 || echo "0")
log_info "Zypper缓存大小: ${ZYPP_CACHE_SIZE}"

if [ -d "/var/cache/zypp/packages" ]; then
    ZYPP_PKG_COUNT=$(find /var/cache/zypp/packages -name "*.rpm" 2>/dev/null | wc -l)
    log_info "缓存包数量: ${ZYPP_PKG_COUNT}"
    if [ "$ZYPP_PKG_COUNT" -le 5 ]; then
        log_success "Zypper缓存已清理"
    else
        log_warning "Zypper缓存未完全清理（${ZYPP_PKG_COUNT}个包）"
    fi
else
    log_success "Zypper缓存目录已清空"
fi
...
log_section "12.2 zypper仓库配置"
log_step "检查仓库配置"
zypper repos 2>&1 | tee -a "$TEST_LOG"
...
# - combustion/config: openSUSE 首次启动配置工具，无配置磁盘时会超时
JOURNAL_FILTERED=$(echo "$JOURNAL_ALL" | grep -Ev "shpchp .*pci_hp_register failed|Slot initialization failed|snd_hda_intel .*no codecs found|agetty\[.*\]: .*failed to get terminal attributes|combustion/config|/dev/combustion" || true)
...
log_section "14.1 RPM 数据库验证"
RPM_VERIFY_OUT=$(rpm -Va 2>&1 | head -20 || true)
if [ -z "${RPM_VERIFY_OUT}" ]; then
    log_success "rpm -Va 未发现异常"
else
    log_warning "rpm -Va 发现部分差异（可能是预期的配置变更）"
    echo "${RPM_VERIFY_OUT}" | head -10 | tee -a "$TEST_LOG"
fi

log_section "14.2 Zypper 数据库状态"
assert_success "zypper --non-interactive refresh" "Zypper仓库刷新"

log_section "14.3 Zypper 锁文件"
ZYPP_LOCK="/var/run/zypp.pid"
if [ -e "$ZYPP_LOCK" ]; then
    LOCK_PID=$(cat "$ZYPP_LOCK" 2>/dev/null || echo "")
    if [ -n "$LOCK_PID" ] && kill -0 "$LOCK_PID" 2>/dev/null; then
        log_warning "Zypper锁被占用: $ZYPP_LOCK (PID: $LOCK_PID)"
    else
        log_success "Zypper锁文件存在但进程未运行"
    fi
else
    log_success "无Zypper锁文件"
fi
...
assert_success "getent hosts download.opensuse.org" "DNS解析 download.opensuse.org"
...
**生成者**: test-image-opensuse.sh v1.0
